    ...
    constructor (baseReporter, config) {
        ...
        this.on('end', () => {
            for (let cid of Object.keys(this.baseReporter.stats.runner)) {
                const capabilities = this.baseReporter.stats.runner[cid]
            ...
            for (let suiteName of Object.keys(spec.suites)) {
                ...
                for (let testName of Object.keys(suite.tests)) {
                    ...
                    test.output.forEach((data) => {
                        if (data.method) {
                            standardOutput.push(`${data.method.toUpperCase()} ${data.uri.href} - ${JSON.stringify(data.data)}`)
                        } else {
                            standardOutput.push(`${(data.requestOptions.method || 'get').toUpperCase()} ${data.requestOptions.uri.href} - ${JSON.stringify(data.body).slice(0, this.errorLogCharacterLimitation)}`)
                        }
                    })
                    testCase.standardOutput(standardOutput.join('\n'))
                }
            }
        }
