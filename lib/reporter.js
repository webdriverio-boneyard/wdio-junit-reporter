...
class JunitReporter extends events.EventEmitter {
    ...
    getStandardOutput (test) {
        ...
        test.output.forEach((data) => {
            switch (data.type) {
            case 'command':
                standardOutput.push(`COMMAND: ${data.command.method.toUpperCase()} ${data.command.uri.href} - ${JSON.stringify(data.command.data)}`)
                break
            case 'result':
                standardOutput.push(`RESULT: ${JSON.stringify(data.result.body).slice(0, this.errorLogCharacterLimitation)}`)
                break
            case 'screenshot':
                standardOutput.push(`[[ATTACHMENT|${data.screenshot.path}]]`)
                break
            }
        })
