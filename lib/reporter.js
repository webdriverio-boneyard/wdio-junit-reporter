...
class JunitReporter extends events.EventEmitter {
    ...
    getStandardOutput (test) {
        ...
        test.output.forEach((data) => {
            switch (data.type) {
            case 'command':
                standardOutput.push(`COMMAND: ${data.command.method.toUpperCase()} ${data.command.uri.href} - ${this.format(data.command.data)}`)
                break
            case 'result':
                standardOutput.push(`RESULT: ${this.format(data.result.body)}`)
                break
            case 'screenshot':
